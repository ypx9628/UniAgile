<template>
  <v-app id="root">
    ...
    <template v-if="$route.path != '/login'">
      <v-navigation-drawer class="d-none d-sm-none d-md-flex " permanent app>
        ...
        <v-list v-for="([text, to, icon, color], index) in links" density="compact" nav>
          <v-list-item v-if="index != 4" :key="text" :to="to" link>
            <v-list-item-icon class="mr-2">
              <v-icon v-text="icon" :color="color"></v-icon>
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="text-button font-weight-black">{{ text }}
              </v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        </v-list>
    ...
    <!-- TopBar -->
    <template v-if="$route.path != '/login'">
      <v-app-bar app elevate-on-scroll class="pr-3" color="#FAFAFA" dense>
        <span class="mx-auto grey--text text-caption font-weight-medium
                        d-flex d-sm-flex d-md-none" style="position:absolute;">{{ time.year }}-{{ time.month }}-{{
                          time.day }}
          {{ time.hour }}:<template v-if="time.min < 10">0</template>{{ time.min }}
        </span>
        <!-- <v-container
        ...
        <v-app-bar-title class="ma-2 text-button  font-weight-black">
          {{ time.year }}/{{ time.month }}/{{ time.day }}
        </v-app-bar-title>

        <span class="pl-3">
          <v-btn color="blue-grey darken-3" fab icon x-small @click="simStart();">
            <v-icon>mdi-timer</v-icon>
          </v-btn>
          <v-btn color="blue-grey darken-3" fab icon x-small @click="simStop()">
            <v-icon>mdi-play-pause</v-icon>
          </v-btn>
          <v-btn color="blue-grey darken-3" fab icon x-small @click="simFaster()">
            <v-icon>mdi-fast-forward</v-icon>
          </v-btn>
          <v-btn color="blue-grey darken-3" fab icon x-small @click="simSlower()">
            <v-icon>mdi-rewind</v-icon>
          </v-btn>
          <v-btn color="blue-grey darken-3" fab icon x-small @click="simRestore()">
            <v-icon>mdi-stop</v-icon>
          </v-btn>
          <v-btn color="blue-grey darken-3" fab icon x-small @click="simInverse()">
            <v-icon>mdi-swap-horizontal-bold</v-icon>
          </v-btn>
          <v-btn color="blue-grey darken-3" fab icon x-small @click="simReset()">
            <v-icon>mdi-restart</v-icon>
          </v-btn>
          <v-btn color="blue-grey darken-3" fab icon x-small @click="simFinish()">
        ...
        <v-menu bottom min-width="140px" rounded-lg offset-y>
          <template v-slot:activator="{ on }">
            <v-btn icon v-on="on">
              <v-avatar color="blue lighten-1" size="34">
          ...
          <v-card>
            <v-list-item-content class="justify-center">
              <div class="mx-auto text-center">
                ...
                <v-btn rounded depressed text @click="Signout()">退出
                </v-btn>
              </div>
            </v-list-item-content>
          </v-card>
    ...
    <template>
      <v-bottom-navigation class="d-flex d-sm-flex d-md-none pt-2" shift app>
        <template>
          <v-btn v-if="index != 4" style="background:none;">
    ...
    <v-main style="background-color:#FAFAFA">
      ...
      <v-container fluid class="ma-0 pa-0">
        <router-view v-if="isReload" class="ma-0 pa-0"></router-view>
      </v-container>
...
export default {
  name: "App",
  data: () => ({
    // sim_start: Vue.prototype.$sim_start,
